...
SELECT student_id, name, score, RANK() OVER (ORDER BY score DESC) AS Student_rank
...
SELECT student_id, UPPER(name) AS UPPERCASE_NAME, monthname(join_date) AS join_month
FROM Student_Performance;
